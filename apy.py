...
def coluna_codigo_populacao(df_pop: pd.DataFrame):
    """Retorna o nome da coluna de código IBGE (6 dígitos, sem DV) no
    arquivo de população, se existir — permite casar município por
    código em vez de nome (evita ambiguidade de nomes repetidos entre
    Estados, ex.: 'Alvorada' existe em vários Estados). Se o CSV for do
    formato antigo (só por nome), retorna None e o app cai automaticamente
    para o casamento por nome (compatibilidade retroativa)."""
    for candidato in ("codigo_ibge", "Codigo_IBGE", "CODIGO_IBGE", "codigo"):
        if candidato in df_pop.columns:
            return candidato
    return None
...
def carregar_populacao(encoding_manual: str | None = None):
    ...
    col_codigo = coluna_codigo_populacao(df)
    if col_codigo:
        # Força texto (senão vira float com linhas de UF/Região em branco
        # virando NaN, e o código de município ganha ".0" no final).
        df[col_codigo] = (
...
def populacao_no_ano(df_pop, col_local_pop, nome_normalizado, ano, anos_pop, nivel=None,
                      codigo=None, col_codigo_pop=None):
    """População de um País/Região/Estado/Município no ano mais próximo
    disponível. Se `codigo` e `col_codigo_pop` forem informados (município,
    casamento por código IBGE — evita nomes repetidos entre Estados), tenta
    casar por código primeiro; só cai para o casamento por nome se não achar
    (compatibilidade com arquivo de população sem coluna de código). Para
    'País': se o arquivo não tiver uma linha explícita de 'Brasil', cai para
    a soma das 27 UFs como aproximação do total nacional."""
    ano_usado = ano_mais_proximo(str(ano), anos_pop)

    if codigo and col_codigo_pop and col_codigo_pop in df_pop.columns:
        linha_codigo = df_pop[df_pop[col_codigo_pop].astype(str).str.strip() == str(codigo).strip()]
        if not linha_codigo.empty:
            return pd.to_numeric(linha_codigo.iloc[0][ano_usado], errors="coerce")
...
def aplicar_taxa(serie_casos: pd.Series, df_pop, col_local_pop, nome_normalizado,
                  anos_pop, base_taxa, nivel=None, codigo=None, col_codigo_pop=None) -> pd.Series:
    ...
    for ts, casos in serie_casos.items():
        pop = populacao_no_ano(df_pop, col_local_pop, nome_normalizado, ts.year, anos_pop, nivel=nivel,
                                codigo=codigo, col_codigo_pop=col_codigo_pop)
...
# ----------------------------------------------------------------------
    ...
    COL_CODIGO_POP = coluna_codigo_populacao(df_pop)
...
# ----------------------------------------------------------------------
    ...
    try:
        ...
        if COL_CODIGO_POP:
            # Casamento por código IBGE (6 dígitos, sem DV) — população de
            ...
            mapa_dados["populacao"] = mapa_dados[COL_UF_MAPA].map(
                lambda nome: pop_lookup.get(normalizar_texto(nome), np.nan)
            )

        areas_sem_pop = int(((mapa_dados["populacao"].isna()) | (mapa_dados["populacao"] <= 0)).sum())
        if areas_sem_pop > 0 and metrica != "Quantidade Absoluta":
            st.warning(f"{areas_sem_pop} município(s) não tiveram população encontrada.")
    ...
    try:
        if tipo_analise != "Tabela de Tendência":
            ...
            if nivel == "País":
                ...
                # Código IBGE (6 dígitos, sem DV) extraído do mesmo texto usado
                # para montar 'id_join' — casamento por código evita pegar a
                # população de outro município com o mesmo nome em outro Estado.
                codigo_mun_escolhido = limpar_codigo(pd.Series([municipio_escolhido])).iloc[0]
            ...
            if tipo_serie == "Número Absoluto":
                ...
                serie = aplicar_taxa(serie_casos, df_pop, COL_LOCAL_POP, nome_pop_normalizado,
                                      ...
                                      codigo=codigo_mun_escolhido if nivel == "Município" else None,
                                      col_codigo_pop=COL_CODIGO_POP)
                if serie.isna().all():
                    nome_buscado = {
                        "País": "Brasil",
